...
def calculate_bleu(pred, gold, rmstop=False):
    return bleu_fromstr([pred], [gold], rmstop=rmstop)


def calculate_exact_match_batch(preds, golds):
    em = 0
...
def calculate_bert_score(pred, gold):
    bert = 0
    device = "mps" if torch.mps.is_available() else "cuda" if torch.cuda.is_available() else "cpu"
    P, R, F1 = score(cands=[pred], refs=[gold], lang="en", device=device)
    bert += F1.sum()
    return bert


def calculate_bert_score_batch(preds, golds, chunk=100):
    bert = 0
    device = "mps" if torch.mps.is_available() else "cuda" if torch.cuda.is_available() else "cpu"
    for i in range(0, len(preds), chunk):
        P, R, F1 = score(cands=preds[i: min(len(preds), i + chunk)], refs=golds[i: min(len(preds), i + chunk)],
